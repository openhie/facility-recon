  ...
  models.MetaDataModel.findOne({}, (err, data) => {    
    ...
    if(data) {
      if(!data.config.hasOwnProperty("generalConfig")) {
        ...
        models.MetaDataModel.findOneAndUpdate({_id: data._id}, 
          {$set: {'config.generalConfig': generalConfig}}, (err, data) => {
        ...
        models.MetaDataModel.findOneAndUpdate({_id: data._id},
          {$set: {'config.generalConfig.reconciliation.parentConstraint': parentConstraint}
        }, (err, data) => {
